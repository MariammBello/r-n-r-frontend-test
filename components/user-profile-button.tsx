...
export default function UserProfileButton() {
  const { user, logout } = useAuth();
  const [firebaseUser] = useAuthState(auth); // ✅ Add firebase user
  ...
  if (!user && !firebaseUser) return null; // ✅ Fallback to firebaseUser

  const avatar =
    user?.avatar || firebaseUser?.photoURL || "/images/placeholder-user.jpg";
  const name = user?.name || firebaseUser?.displayName || "User";
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <div className="flex flex-col items-start">
          <div className="flex items-center">
            <span className="text-[#0e2f3c] font-medium text-sm">{name}</span>
            {verified && (
              <div className="ml-1 w-4 h-4 bg-green-500 rounded-full flex items-center justify-center">
                <svg
                  width="10"
          ...
          <span className="text-[#4f4f4f] text-xs">{role}</span>
        </div>
        <ChevronDown
          size={16}
          className={`text-[#0e2f3c] transition-transform ${
            isOpen ? "rotate-180" : ""
          }`}
        />
      </button>
        ...
        <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-100 z-50">
          <div className="p-4 border-b border-gray-100">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="flex items-center">
                  <h3 className="font-bold text-[#0e2f3c]">{name}</h3>
                  {verified && (
                    <div className="ml-1 w-4 h-4 bg-green-500 rounded-full flex items-center justify-center">
                      <svg
                        width="10"
                ...
                <p className="text-[#4f4f4f] text-sm">{email}</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 py-2">
            <button
              onClick={() => {
                logout?.() || auth.signOut(); // ✅ support both logout functions
                setIsOpen(false);
              }}
              className="flex items-center w-full text-left px-4 py-2 text-red-600 hover:bg-gray-50 transition-colors"
            >
